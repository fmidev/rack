...
function TEST_TIFF(){
    ...
    echo "# GDALINFO"
    cmd="gdalinfo $outfile > $inffile.tmp"
    echo $cmd
    eval $cmd 
    if [ $? != 0 ]; then
	echo "GDAL error with projdef='$projdef' ($outfile)"
    fi
    cat $inffile.tmp | fgrep -v 'Files:' > $inffile
    #echo $cmd
...
for EPSG in ${ARGS[*]}; do
    ...
    TEST_TIFF "+init=epsg:$EPSG" test-init-$EPSG.tif

    TEST_TIFF "$PROJDEF" test-projdef-$EPSG.tif
