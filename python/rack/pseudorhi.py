...
def get_background_filename(args) -> str:
    return f"{args.basename}-background.png"
...
def handle_outfiles_prhi(args, cmdBuilder: rack.core.Rack) -> str:
    ...
    cmdBuilder.gLinkImage(f"{args.gnuplot}")
    ...
    bg_filepath = get_background_filename(args)
    cmdBuilder.outputFile(bg_filepath)
    
    if 'png' in args.FORMAT:
        #cmdBuilder.outputFile(f"{args.basename}.h5")
        p = Path(args.OUTDIR, f"{args.basename}.png")
        logger.info(f"Linking explicitly requested PNG image: {p}")
        if not p.exists():
            #target = Path(args.OUTDIR).resolve()
            p.symlink_to(Path(args.OUTDIR, bg_filepath).resolve())
        else:
            logger.info(f"Link file exists already: {p}")
        args.FORMAT.remove('png')
...
def handle_gnuplot(args, progBuilder: rack.core.Rack): #, **kw_args): #range_m:tuple=None, height_m:tuple=None):
    ...
    plotScript, plotScriptBuilder = rack.vertical.gnuplot_new_script()

    # conf.comment("Generated by rack with --gnuplot option")
    # conf.comment("General settings")
    plotScriptBuilder.terminal(rack.gnuplot.Terminal(terminal), size=args.size) # Hard-coded. See also handle_prhi for size handling.
    # p = Path(args.OUTDIR, args.gnuplot)
    # c
    plotScriptBuilder.output(args.gnuplot)

    plotScriptBuilder.multiplot()    # (rows=1, cols=1)
    if args.title:
        plotScriptBuilder.title(args.title)
    elif args.select:
        plotScriptBuilder.title(f"Pseudo-RHI [{args.select}] {args.az_angle} deg")
    elif args.title is None:
        plotScriptBuilder.title(f"Pseudo-RHI {args.az_angle} deg")


    plotScriptBuilder.unset("tics")
    plotScriptBuilder.grid(rack.gnuplot.Tics.XTICS, rack.gnuplot.Tics.YTICS)
    plotScriptBuilder.unset("border")

    # consider single command with multiple parameters for margins, e.g. margin left 10, right 20, top 30, bottom 40
    """
    plotScriptBuilder.comment("lmargin at screen 0.22")
    plotScriptBuilder.comment("rmargin at screen 0.9")
    plotScriptBuilder.comment("bmargin at screen 0.18")
    plotScriptBuilder.comment("tmargin at screen 0.9")
    """
    
    # left, right, bottom, top in character units. See also margin command for more flexible margins in screen units.
    plotScriptBuilder.set("margins 12, 4, 4, 3"  ) 

    # plotScriptBuilder.comment("General settings")
    
    plotScriptBuilder.comment("Plotting the background image (radar beams)")
    ...
    plotScriptBuilder.xrange(xrange)
    plotScriptBuilder.yrange(yrange)

    # See above unset border, to remove default border and tics,
    # and then add custom border back with margins
    plotScriptBuilder.set("border") 
    plotScriptBuilder.set("tics out nomirror scale 2")
    plotScriptBuilder.set("mxtics 5")
    # plotScriptBuilder.key(rack.gnuplot.Key.LEFT, outside=True)
    plotScriptBuilder.xlabel(f'Range {xrange[0]} – {xrange[1]} m')
    plotScriptBuilder.ylabel(f'Altitude {yrange[0]} – {yrange[1]} m')
    ...
    if args.print:
        ...
        print(script)
    rack.vertical.gnuplot_write_script(args, script)
