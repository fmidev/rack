...
def build_parser():
    ...
    parser.add_argument(
        "--RANGE",
        metavar="<max>|<min>:<max>",
        help="Radial range [metres]")  

    parser.add_argument(
        "--AZIMUTH",
        metavar="<angle>",
        help="Azimuthal direction [degrees]")  
    ...
    parser.add_argument(
        "--BEAM_WIDTH",
        default="MAXIMUM",
        help="Width of beam [metres].") 

    parser.add_argument(
        "--WEIGHT_THRESHOLD",
        metavar="<value>",
        help="0-1 threshold for weight field, for masking output") 
    """
...
# TODO: move to separate file, and also support for multiple SELECT-like parameters (e.g. quantity and dataset)
"""
...
def handle_gnuplot(args, progBuilder: rack.core.Rack):
    ...
    # TODO: ensure png file is generated by rack, and use that as input for gnuplot (background image)

    prog_conf = rack.gnuplot.ConfSequence()

    conf = rack.gnuplot.Registry(prog_conf)
    #reg.terminal(GnuPlot.Terminal.PNG, size=(800, 600))
    conf.terminal(rack.gnuplot.Terminal.PNG, size=(800, 600))
