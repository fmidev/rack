...
# RACK=${RACK:-'rack --odim 2.2'} # disturbs doc examples...
RACK=${RACK:-'rack'}
...
DEMOFILES=${DEMO:+"$NEWLINE --gInclude NONE -o $BASENAME.png -o $BASENAME.tif -Q QIND -o $BASENAME-QIND.png -Q COUNT -o $BASENAME-COUNT.png -Q '.*DEV' -o $BASENAME-STDEV.png $NEWLINE -Q '' --palette '${PALETTE}' $grid --gInclude NEXT -o $BASENAME-rgb.png --gAlign TOP,RIGHT --legendOut $BASENAME-leg.svg $NEWLINE --imageTransp 0.1:0.2,0.3,0.7 -o $BASENAME-rgba.png $NEWLINE   -o $BASENAME.svg $NEWLINE  -o $BASENAME-h5.txt"}
...
command="$command $NEWLINE$encoding --cExtract DATA,WEIGHT,COUNT $NEWLINE --gTitle 'Rack Composite - \${what:date|%Y/%m}' -o $OUTFILE $DEMOFILES"
